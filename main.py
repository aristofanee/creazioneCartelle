...
class MainWindow(QMainWindow):
    ...
    def save_spec_file(self):
        ...
        QMessageBox.information(
            self, "vehicle_spec.txt", "The file was correctly saved."
        )
    ...
    def button_pressed(self):
        ...
        if (
            self.ui.textbox_model.text().strip() == ""
            or self.ui.textbox_year.text().strip() == ""
            or self.ui.textbox_oem.text().strip() == ""
            or self.ui.textbox_number.text().strip() == ""
        ):
            QMessageBox.warning(
                self, "Warning", "Some of the specifications are missing"
            )
            return
        ...
        # Check what type of vehicle the subject is (Car/Van/Truck)
        self.encapVehicle = "C"
        if self.ui.radio_van.isChecked():
            self.encapVehicle = "V"        
        elif self.ui.radio_truck.isChecked():
            self.encapVehicle = "T"
        ...
        for checkbox in checkboxes:
            ...
            if test_name[0] == "C":
                test_name = self.encapVehicle + test_name[1:]
            ...
            mme_file_lines = mme_processor.mmefile_creator(
                self.vehicle_spec_file,
                year,
                model,
                folder_name,
                test_name,
            ...
            # Saves the .mme file
            with open(
                test_path / (folder_name + ".mme"), "w", encoding="utf-8"
            ) as file:
                for line in mme_file_lines:
                    file.write(line + "\n")

        QMessageBox.information(
            self, "Euro NCAP foldering", "The folders were created correctly."
        )
